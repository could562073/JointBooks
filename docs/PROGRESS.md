...
目前測試：**Vitest 1293（141 檔）、Playwright 133（ip13）／404（三個寬度＋正式建置）**，typecheck 兩個 project 都乾淨，
...
- **記一筆的鍵盤跑出畫面**（使用者回報，2026-09-18）：iPhone 15 Pro Max 上 0 那排在畫面外，iPhone 13 剛好放得下。量截圖發現整頁被放大約 1.15 倍（430÷375），使用者確認開了「顯示放大：較大文字」：網頁被當成 375×812 排版，扣掉狀態列可用高度約 765，比 iPhone 13 還矮；面板內容（手機上約 850）超過 96dvh 的上限，而原本是整片面板一起捲，鍵盤就被捲到下面（往上滑捲得到，確認不是 iOS 算錯高度）。改成鍵盤包在 `.dock` 裡 `position: sticky; bottom: 0`，面板底部留白移進底座：裝得下時外觀不變，裝不下時鍵盤貼底、上面的內容從後面捲過去。`entry.spec.ts` 用 375×700 檢查沒捲時 0 那排與儲存鍵在畫面內、捲到底時備註欄不被鍵盤蓋住。
- **不登入使用與登入／登出**（使用者要求，2026-09-18）：開始畫面可以「先不登入，直接使用」，帳只存在這台手機；配置頁最上面的帳號區可以登入、登出。登出後帳留在手機上變回本機模式，同帳號再登入接回那本帳（受邀者接回對方的帳本）；登出會撤銷 Google 的授權，下次登入重新同意一次（授權碼流程只有同意時才發續期憑證）。換了帳號、或帳號雲端已經有帳本而手機上也有帳時，跳確認視窗選合併／改用雲端／取消；手機上的帳混著兩個人記的不能合併。合併時分類依「收支類型＋名稱」對到那本帳（`sync/categoryRemap.ts`），只帶有帳在用的分類，帳改算成自己在那本帳的身分，成員名稱以雲端為準。帳號用 Drive `about.get` 的 permissionId 比對，不多要權限；上線前就登入的手機第一次連上時補記。登入判斷是純函式 `planSignIn`（`sync/account.ts`），`App.tsx` 的 `Gate` 持有接上的帳本、`Shell` 的同步依 sid 重新啟動。設計見 `docs/superpowers/specs/2026-09-18-guest-mode-and-account-design.md`，實作計畫見 `docs/superpowers/plans/2026-09-18-guest-mode-and-account.md`。
- ~~**其中稅**（對加拿大收據用，2026-09-22）：記一筆時可以多填「其中稅（可不填）」，金額欄仍是收據的總額，不是拆出來的稅前金額；稅只是註記其中有多少是稅，「稅前」＝金額－稅，即時算給你看（`稅前 $45.87`），存的時候只是額外顯示，不影響任何統計。稅存在 `Txn.taxCents`，寫進 Sheet 紀錄頁第 15 欄 O「稅」（同步與建帳本的範圍已改成 A–O，舊帳本第一次推送前補寫 O1 標頭）；記一筆／編輯面板多一張稅卡（testid `field-tax`），稅大於金額時卡片下方出現「稅不能大於金額」（`tax-error`）且不顯示稅前金額。`e2e/entry.spec.ts` 新增一支涵蓋輸入、稅前試算、存檔後重新打開稅還在。~~
  已被下面 2026-09-23（v1.4.0）那條取代：金額欄改成直接填稅前，不再是「金額仍是總額、稅只是註記」。
- ~~**沒有登出功能。**~~ 配置頁可以登出了（見上方「不登入使用與登入／登出」）。登出會一併撤銷 Google 的授權。
