...
| 登入頁、接受邀請頁 | ✅ |
| 邀請面板（InvitePanel） | ✅（說明卡有兩句刻意不照原型，見下方） |
...
**邀請面板有兩句說明刻意不照原型（待使用者裁決）。** 原型寫「過期後可重新產生，舊連結立即失效」與「只有你能移除成員」，兩句在這個 App 都不成立：沒有後端，產生新連結不會讓已傳出去的舊連結失效；App 也沒有移除成員的功能。改寫成事實，並補回原型拿掉的「任何拿到這條連結的人都能加入」提醒——`InvitePanel.test` 有一條測試擋住「舊連結立即失效」被改回去。另外「會被加為這份 Sheet 的編輯者」是 §8 的設計，但加入流程目前還沒真的去改 Drive 權限（帳本建立流程尚未接上）。
