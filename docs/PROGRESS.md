...
- **不登入使用與登入／登出**（使用者要求，2026-09-18）：開始畫面可以「先不登入，直接使用」，帳只存在這台手機；配置頁最上面的帳號區可以登入、登出。登出後帳留在手機上變回本機模式，同帳號再登入接回那本帳（受邀者接回對方的帳本）；登出只清掉這台手機的通行證，不撤銷 Google 的授權（`TokenProvider.disconnect()` 兩種登入方式都不再 revoke）。換了帳號、或帳號雲端已經有帳本而手機上也有帳時，跳確認視窗選合併／改用雲端／取消；手機上的帳混著兩個人記的不能合併。合併時分類依「收支類型＋名稱」對到那本帳（`sync/categoryRemap.ts`），只帶有帳在用的分類，帳改算成自己在那本帳的身分，成員名稱以雲端為準。帳號用 Drive `about.get` 的 permissionId 比對，不多要權限；上線前就登入的手機第一次連上時補記。登入判斷是純函式 `planSignIn`（`sync/account.ts`），`App.tsx` 的 `Gate` 持有接上的帳本、`Shell` 的同步依 sid 重新啟動。設計見 `docs/superpowers/specs/2026-09-18-guest-mode-and-account-design.md`，實作計畫見 `docs/superpowers/plans/2026-09-18-guest-mode-and-account.md`。
- ~~**沒有登出功能。**~~ 配置頁可以登出了（見上方「不登入使用與登入／登出」）。登出不撤銷 Google 的授權，要撤銷得到 Google 帳號的「第三方應用程式」移除。
