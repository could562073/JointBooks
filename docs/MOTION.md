...
| 39 | 啟動畫面（原型 v2 新增：兩顆饅頭依序彈起、光暈擴散、字標浮現（原型底部的讀取條依使用者要求拿掉）；0.9s 放大淡出、淡出播完卸載；減少動態時淡出 120ms。使用者要求，歸 App 外殼） | `DUR.bootHold` `DUR.bootOut` `DUR.bootBun` `DUR.bootBunStagger` `DUR.bootWord` `DUR.bootWordDelay` `DUR.bootRing` `EASE.enter` | 01 | 已實作 |
